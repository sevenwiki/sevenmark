use crate::ast::{ListItem, ListItemChild, Location};
...
pub fn list_core_parser(parser_input: &mut ParserInput) -> Result<Vec<ListItemChild>> {
    repeat(1.., list_item_child_parser).parse_next(parser_input)
}

/// 리스트 콘텐츠 아이템 파서 (아이템 또는 조건부)
fn list_item_child_parser(parser_input: &mut ParserInput) -> Result<ListItemChild> {
    alt((
        list_item_parser.map(ListItemChild::Item),
...
fn list_conditional_parser(parser_input: &mut ParserInput) -> Result<ListItemChild> {
    ...
    let items: Vec<ListItem> = repeat(0.., list_item_parser).parse_next(parser_input)?;
...
fn list_item_parser(parser_input: &mut ParserInput) -> Result<ListItem> {
    let start = parser_input.input.current_token_start();

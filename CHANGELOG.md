...
### Changed
- **sevenmark_lsp_core**: Table parameter completions updated to reflect the renamed wrapper parameters: `align` and `width` replaced by `wrapper-align`, `wrapper-width`, `wrapper-style`, and `wrapper-dark-style`.
...
### Changed
Light(base) mode style parameters now register hash-keyed shared rules in a document-level stylesheet.
...
### Added
- **sevenmark_lsp_core**: Table parameter completions now include `#width`
...
### Changed
- **sevenmark_ast**: Replaced `Parameters` storage with `IndexMap<String, Parameter>` so parameter order now follows the source document while preserving keyed lookup semantics
- **sevenmark_parser**: Parameter parsing now collects into insertion-ordered `Parameters`, making serialized AST parameter order match the user-written order instead of alphabetical key order

### Added
- **docs**: Updated the AST API documentation to describe insertion-ordered parameter storage
- **tests**: Regenerated parser fixture expectations for parameter order preservation
...
### Fixed
- **sevenmark_wasm**: Corrected invalid MediaWiki-style header syntax (`== SevenMark ==`) to SevenMark syntax (`## SevenMark`) in npm package README examples
...
### Fixed
- **sevenmark_lsp_core**: Corrected brace keyword completion for block quotes from `blockquote` to `quote` so `{{{#` autocomplete matches parser-supported syntax
...
### Changed
- **sevenmark_parser**: Parameter keys now support Unicode alphanumeric characters and underscores — `#한국어="value"`, `#key_name="value"` are now valid in addition to ASCII-only keys
...
### Fixed
- **Dockerfile**: Added example stub creation before `cargo chef cook` to fix build failure caused by `[[example]]` with `required-features`
- **sevenmark_lsp_core**: Added `folded_header_produces_folded_header_token` regression test for `FoldedHeader` semantic token
...
### Added
- **sevenmark_ast**: Crate-level `README.md` for crates.io publishing
- **sevenmark_language_server**: Crate-level `README.md` for crates.io publishing
...
### Changed
- **CI / Editor Releases**: Unified editor plugin versioning with release tags
  - JetBrains plugin workflow now derives `pluginVersion` from the Git tag (`vX.Y.Z`) at release time
  - VS Code extension workflow now syncs `package.json` version from the same release tag before packaging
  - Removes manual editor plugin version drift during tagged releases

## [2.24.14] - 2026-02-19

### Changed
- **sevenmark_language_server**: Improved JetBrains semantic highlighting contrast for structural delimiters
  - Remapped `tableRow`, `tableCell`, `listItem`, and `foldInner` semantic token categories from `property` to `operator`
  - Kept token index ordering stable to preserve semantic token encoding compatibility
...
### Added
- **Discord widget embed support**: New `[[#discord ...]]` syntax for embedding Discord server widgets
  - `[[#discord #id="SERVER_ID"]]` - Server widget (light theme)
  - `[[#discord #id="SERVER_ID" #dark]]` - Dark theme
  - Options: `#width`, `#height` (default: 350x500)
...
### Added
- **YouTube playlist support**: New `#playlist` parameter for embedding YouTube playlists
  - `[[#youtube #playlist="PLxxxxx"]]` → embeds full playlist
  - `[[#youtube #id="..." #playlist="PLxxxxx"]]` → starts at specific video in playlist
...
- **Direct iframe rendering**: Videos render as `<iframe>` with lazy loading (no client-side JS required)
...
### Breaking Changes
- **sevenmark-parser**: `NewLine` element replaced with `SoftBreak` and `HardBreak`
  - `SoftBreak`: from regular `\n` - context-dependent rendering
  - `HardBreak`: from `[br]` macro - always renders as `<br>`

### Changed
- **sevenmark-html**: `SoftBreak` rendering is context-aware
  - Inside brace elements (styled, blockquote, table, list, fold, etc.): ignored
  - Outside brace elements: renders as `<br>`
  - `HardBreak` always renders as `<br>`

### Added
- **sevenmark-html**: `suppress_soft_breaks_depth` in `RenderContext`
  - Helper methods: `enter_suppress_soft_breaks()`, `exit_suppress_soft_breaks()`, `is_soft_break_suppressed()`

## [2.9.0] - 2025-12-31

### Changed
- **sevenmark-parser**: Trailing whitespace before closing delimiters (`}}}`, `]]`) is now trimmed
  - Regular newlines (`\n`) before `}}}` or `]]` are consumed by suffix parser
...
### Added
- **sevenmark-html**: Folded header (`#!`) now renders as `<details>/<summary>`
  - Native browser collapse/expand without JavaScript
  - `<details class="sm-section sm-folded">` replaces `<section>` for folded headers
  - Header wrapped in `<summary>` tag
  - New CSS class `SECTION_FOLDED` added to `classes.rs`
...
### Changed
- **sevenmark-html**: Table now wrapped in `div.sm-table-wrapper`
  - Enables `overflow-x: auto` for horizontal scrolling on small screens
  - New CSS class `TABLE_WRAPPER` added to `classes.rs`
